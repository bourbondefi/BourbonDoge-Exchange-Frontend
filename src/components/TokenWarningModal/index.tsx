        ...
        <AutoColumn gap="10px" justify="flex-start">
          ...
          {chainId && (
            <ExternalLink style={{ fontWeight: 400 }} href={getBscScanLink(chainId, token.address, 'token')}>
              <Text title={token.address}>{shortenAddress(token.address)} (View on DOGE Scan)</Text>
            </ExternalLink>
          )}
        </AutoColumn>
      </AutoRow>
    ...
    <Modal isOpen={isOpen} onDismiss={handleDismiss} maxHeight={90}>
      <WarningContainer className="token-warning-container">
        <AutoColumn gap="lg">
          ...
          <Text>
            Anyone can create an BEP20 token on DOGE with <em>any</em> name, including creating fake versions of existing
